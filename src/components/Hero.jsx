import { Link } from "react-router-dom";
import affiche from "../assets/affiche.png";

function Hero() {
  return (
    <section
      className="
        bg-gradient-to-br
        from-red-700
        via-red-600
        to-black
        text-white
        overflow-hidden
      "
      aria-labelledby="home-hero-title"
    >
      <div className="relative z-10 max-w-7xl mx-auto px-6 md:px-8 py-16 md:py-24">

        {/* HERO PRINCIPAL */}
        <div className="grid lg:grid-cols-2 gap-16 items-center">

          {/* TEXTE */}
          <div>

            <p className="uppercase tracking-[6px] text-red-100 mb-6">
              GPS • IoT • Gestion de flotte • Télématique
            </p>

            <h1
              id="home-hero-title"
              className="
                text-4xl
                sm:text-5xl
                lg:text-7xl
                font-extrabold
                leading-tight
              "
            >
              Solutions GPS
              <br />
              et Gestion de Flotte
              <br />
              à Kinshasa et en RDC
            </h1>

            <p
              className="
                mt-8
                text-lg
                md:text-xl
                text-gray-100
                leading-7
                md:leading-8
              "
            >
              HESABU SARLU accompagne les entreprises, institutions et particuliers
              à Kinshasa et partout en République Démocratique du Congo avec des
              solutions de géolocalisation GPS, de suivi de véhicules, de télématique
              et de gestion intelligente de flotte.
            </p>

            {/* BOUTONS */}
            <div
              className="
                flex
                flex-col
                sm:flex-row
                gap-5
                mt-10
              "
            >
              <Link
                to="/devis"
                className="
                  w-full
                  sm:w-auto
                  bg-white
                  text-red-700
                  px-8
                  py-4
                  rounded-xl
                  font-bold
                  shadow-xl
                  hover:scale-105
                  focus:outline-none
                  focus:ring-4
                  focus:ring-red-300
                  transition
                  duration-300
                "
              >
                Demander un devis
              </Link>

              <Link
                to="/client"
                className="
                  w-full
                  sm:w-auto
                  border-2
                  border-white
                  px-8
                  py-4
                  rounded-xl
                  font-bold
                  hover:bg-white
                  hover:text-red-700
                  focus:outline-none
                  focus:ring-4
                  focus:ring-white/60
                  transition
                  duration-300
                "
              >
                Espace Client
              </Link>
            </div>

            {/* STATS */}
            <div
              className="
                grid
                grid-cols-1
                sm:grid-cols-3
                gap-8
                mt-12
                md:mt-16
                text-center
                sm:text-left
              "
            >
              <div>
                <p className="text-4xl font-bold">
                  +50
                </p>

                <p className="text-gray-100">
                  Véhicules suivis
                </p>
              </div>

              <div>
                <p className="text-4xl font-bold">
                  24/7
                </p>

                <p className="text-gray-100">
                  Surveillance
                </p>
              </div>

              <div>
                <p className="text-4xl font-bold">
                  99%
                </p>

                <p className="text-gray-100">
                  Satisfaction
                </p>
              </div>
            </div>

          </div>

          {/* IMAGE */}
          <div className="flex justify-center">
            <img
              src={affiche}
              alt="Solutions GPS et gestion de flotte HESABU à Kinshasa en RDC"
              fetchPriority="high"
              decoding="async"
              className="
                w-full
                max-w-2xl
                rounded-3xl
                shadow-2xl
                hover:scale-105
                transition
                duration-500
              "
            />
          </div>

        </div>
      </div>

      {/* APERÇU RAPIDE DES RUBRIQUES */}

      <div className="relative z-10 max-w-7xl mx-auto px-6 md:px-8 pb-16 md:pb-20">

        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">

          <Link
            to="/services"
            className="
              bg-white/10
              backdrop-blur-md
              p-6
              rounded-2xl
              border
              border-white/20
              hover:bg-white/20
              focus:outline-none
              focus:ring-4
              focus:ring-white/50
              transition
            "
          >
            <div
              className="text-4xl mb-4"
              aria-hidden="true"
            >
              🛰️
            </div>

            <h3 className="text-2xl font-bold mb-2">
              Services GPS
            </h3>

            <p className="text-red-100">
              Suivi GPS, géolocalisation, anti-vol,
              alertes intelligentes et maintenance
              des véhicules.
            </p>
          </Link>

          <Link
            to="/solutions"
            className="
              bg-white/10
              backdrop-blur-md
              p-6
              rounded-2xl
              border
              border-white/20
              hover:bg-white/20
              focus:outline-none
              focus:ring-4
              focus:ring-white/50
              transition
            "
          >
            <div
              className="text-4xl mb-4"
              aria-hidden="true"
            >
              🚚
            </div>

            <h3 className="text-2xl font-bold mb-2">
              Gestion de flotte
            </h3>

            <p className="text-red-100">
              Solutions intelligentes pour gérer,
              suivre et sécuriser les flottes
              automobiles en RDC.
            </p>
          </Link>

          <Link
            to="/about"
            className="
              bg-white/10
              backdrop-blur-md
              p-6
              rounded-2xl
              border
              border-white/20
              hover:bg-white/20
              focus:outline-none
              focus:ring-4
              focus:ring-white/50
              transition
            "
          >
            <div
              className="text-4xl mb-4"
              aria-hidden="true"
            >
              🏢
            </div>

            <h3 className="text-2xl font-bold mb-2">
              À propos de HESABU
            </h3>

            <p className="text-red-100">
              Découvrez notre expertise en GPS,
              télématique, IoT et solutions
              de mobilité connectée.
            </p>
          </Link>

          <Link
            to="/contact"
            className="
              bg-white/10
              backdrop-blur-md
              p-6
              rounded-2xl
              border
              border-white/20
              hover:bg-white/20
              focus:outline-none
              focus:ring-4
              focus:ring-white/50
              transition
            "
          >
            <div
              className="text-4xl mb-4"
              aria-hidden="true"
            >
              📞
            </div>

            <h3 className="text-2xl font-bold mb-2">
              Contact
            </h3>

            <p className="text-red-100">
              Contactez HESABU pour un devis,
              une démonstration ou un projet
              de gestion de flotte en RDC.
            </p>
          </Link>

        </div>
      </div>

      {/* VAGUE SVG */}

      <div className="overflow-hidden leading-none">
        <svg
          viewBox="0 0 1440 120"
          className="w-full"
          aria-hidden="true"
        >
          <path
            fill="#ffffff"
            d="M0,64L80,80C160,96,320,128,480,122.7C640,117,800,75,960,64C1120,53,1280,75,1360,85.3L1440,96L1440,160L0,160Z"
          />
        </svg>
      </div>

    </section>
  );
}

export default Hero;
